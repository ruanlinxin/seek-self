 ...
 * Peer 状态
 */
export type PeerStatus = 'idle' | 'initializing' | 'ready' | 'connecting' | 'error' | 'closed';
...
export type EventType = 
  ...
  | 'peer:initializing'   // Peer 初始化中
  | 'peer:cleanup'        // Peer 清理
  | 'connection:open'     // 连接建立
  | 'connection:close'    // 连接关闭
  | 'connection:error'    // 连接错误
  | 'connection:connecting'   // 连接中
  | 'connection:incoming' // 传入连接
 ...
 * 事件监听器
 */
export type EventListener<T = any> = (event: EventData<T>) => void;
...
export interface PeerHookReturn {
  ...
  /** 发送确认 */
  sendAck: (messageId: string, peerId: string, status: PeerMessage['ackStatus']) => void;
